...
/*   Updated: 2024/10/27 18:08:10 by ryusupov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	*set_color(char *map, t_data *data)
{
	int		*color;
	int		i;
	char	**rgb_code;
...
int	rgb_codes(t_data *data, char **rgb)
{
	...
	while (rgb[i])
	{
		if (rgb[i][0] == 'C')
		{
			data->texture.rgb_cell = set_color(rgb[i] + 1, data);
			if (data->texture.rgb_cell == 0)
				return (error_msg("rgb color error.", 1));
		}
		else if (rgb[i][0] == 'F')
		{
			data->texture.rgb_floor = set_color(rgb[i] + 1, data);
